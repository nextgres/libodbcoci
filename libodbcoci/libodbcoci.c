...
static bool _libodbcoci_is_initialized = false;
static void * _libodbcoci_dlhandle = NULL;
static libodbcoci_config_t _libodbcoci_configuration = {
...
#define DEFINE_MYSQL_FPTR(name)                                         \
  libodbcoci_##name##_fptr_t libodbcoci_dlsym_##name = NULL;
...
_libodbcoci_inih_cb (
  ...
  libodbcoci_config_t *cfg = (libodbcoci_config_t *) user;
...
void
_libodbcoci_load_configuration (
  void
) {
  char *ini_file_path = getenv("LIBODBCMYSQL_INI");

  if (NULL != ini_file_path) {
    if (0 > ini_parse(ini_file_path, _libodbcoci_inih_cb,
      &_libodbcoci_configuration)) {
    ...
    for (int ii = 0; ii < (sizeof(search_path) / sizeof(char *)); ++ii) {
      printf("parsing %s\n", search_path[ii]);
      int rc = ini_parse(search_path[ii], _libodbcoci_inih_cb,
        &_libodbcoci_configuration);
      printf("rc = %d\n", rc);
      if (0 == rc) {
  ...
  int log_level = (int) strtol(_libodbcoci_configuration.default_log_level,
    (char **) NULL, 10);
  switch (log_level) {
    case LOG_TRACE:
    case LOG_DEBUG:
    ...
    default:
      log_error("Invalid log level (%s)",
        _libodbcoci_configuration.default_log_level);
      log_set_level(LOG_ERROR);
  }

  /* Dump the config we've read */
  dump_config(&_libodbcoci_configuration);

  /* Load dynamic */
  if (NULL != _libodbcoci_configuration.default_dlopen) {
    _libodbcoci_dlhandle = dlopen(_libodbcoci_configuration.default_dlopen,
      (RTLD_LOCAL | RTLD_LAZY));
    if (NULL != _libodbcoci_dlhandle) {
      #define ASSIGN_MYSQL_FPTR(name)                                         \
        do {                                                                  \
          if (NULL != _libodbcoci_configuration.name##_dlsym) {             \
            MYSQL_FPTR(name) = MYSQL_FPTR_TYPE(name)                          \
              dlsym(_libodbcoci_dlhandle, #name);                           \
...
libodbcoci_init (
  ...
  if (true == _libodbcoci_is_initialized) {
    return;
  }
  ...
  /* One round only */
  _libodbcoci_is_initialized = true;
